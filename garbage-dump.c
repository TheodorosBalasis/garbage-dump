...
void generate_random_string(char* buffer, size_t buffer_size, char lower_bound, char upper_bound);
			...
			if (chunk_size) {
				...
				if (filename && chars) {
					for (int i = 0; i < number_of_files; i++) {
						...
						generate_random_string(filename, FILENAME_LENGTH, FILENAME_LEAST_CHAR, FILENAME_GREATEST_CHAR);
